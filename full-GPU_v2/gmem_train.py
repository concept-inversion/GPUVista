...
def save_model(name, model):
    traced_script_module = torch.jit.trace(model, torch.rand(1, GMEM_FEATURE*GMEM_CONTEXT).to(device))
...
def load_data(dataset):
    ...
    inp_data= inp_data.reshape(-1,GMEM_FEATURE,GMEM_CONTEXT)
...
if __name__ == '__main__':
    ...
    model=CNN_smem(GMEM_INPUT_SIZE, 1)
    model.to(device)
